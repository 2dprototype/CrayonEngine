...
-- Overhauled physics, time dilation, and procedural explosion manifolds
-- ============================================================================
...
function crayon.update(raw_dt)
    ...
    if crayon.input.is_down("w") then cam.z = cam.z - move_spd end
    if crayon.input.is_down("s") then cam.z = cam.z + move_spd end
    if crayon.input.is_down("a") then cam.x = cam.x - move_spd end
    if crayon.input.is_down("d") then cam.x = cam.x + move_spd end
...
function crayon.draw()
    ...
    crayon.graphics.draw_model(models.plane, 0, 0, 0, 0, 0, 0, 35.0, 1.0, 35.0, tex_white)
